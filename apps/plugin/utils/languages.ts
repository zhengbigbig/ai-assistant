...
const UI_LANGUAGES = Object.keys(ALL_LANGUAGES_NAMES);
const TARGET_LANGUAGES = Object.keys(ALL_LANGUAGES_NAMES['en']);
...
export const languages = {
   ...
   * @returns {string}
   */
  fixUILanguageCode: (langCode: string) => {
    if (typeof langCode !== 'string') return;

    function getReplacer(langCode: string) {
      switch (langCode) {
        case 'pt':
          return 'pt-BR';
        case 'zh':
    ...
    if (UI_LANGUAGES.indexOf(langCode) === -1) {
   ...
   * @returns {string}
   */
  fixTLanguageCode: (langCode: string) => {
    if (typeof langCode !== 'string') return;
    ...
    if (TARGET_LANGUAGES.indexOf(langCode) === -1) {
      if (langCode.indexOf('-') !== -1) {
        langCode = langCode.split('-')[0];
        if (TARGET_LANGUAGES.indexOf(langCode) === -1) {
          return;
  ...
  getLanguageList: () => {
    ...
    uiLanguage = languages.fixUILanguageCode(uiLanguage) || "en";
    return ALL_LANGUAGES_NAMES[uiLanguage as keyof typeof ALL_LANGUAGES_NAMES];
  },
  /**
   * 获取替代服务
   * @param {string} lang
  ...
  getAlternativeService: (
    ...
    for (const sn in SUPPORTED_LANGUAGES) {
      ...
      if (forPageTranslation && pageTranslationServices.indexOf(sn) === -1)
        continue;
      const langs = SUPPORTED_LANGUAGES[sn as keyof typeof SUPPORTED_LANGUAGES];
  ...
  codeToLanguage: (langCode: string) => {
    ...
    langCode = languages.fixTLanguageCode(langCode) || "und";
    return langCode ? languageList[langCode as keyof typeof languageList] : "";
  },
};
